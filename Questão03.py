"""
O IBGE (Instituto Brasileiro de Geografia e Estatística)
para definir um preço médio de um produto alimentar,
por exemplo: o feijão, faz consulta a pelo menos 5 mercados
de tamanhos variados. Após ler os valores dos 5 mercados,
o instituto calcula o valor médio do produto. Diferente
do que era praticado antes, atualmente, o IBGE utiliza
um pequeno sistema para computar o preço médio.
Você está trabalhando na equipe de TI do IBGE para
calcular o preço médio dos produtos. Lembre-se de
ler os cinco preços, calcular a média e exibir o resultado.
"""


produto = input("Qual é o produto?")

preco = []

for i in range(5):
    valor = float(input("Qual o valor do produto?"))
    preco.append(valor)

media = sum(preco) / len(preco)

print(f"\nO preço médio do {produto} é: R$ {media:.2f}")

